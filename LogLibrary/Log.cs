...
namespace LogLibrary
{
    public static class Log
    {
        ...
        private static string sqllog =
            "INSERT INTO log" +
            "(datetimelog, pcname, command, insertedid) " +
            "VALUES('{0}', '{1}', '{2}', {3})";

        public static void Add(string sqlstring, int id)
        {
            string dtime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

            sqlstring = Parse(sqlstring);

            string sql = String.Format(
                sqllog, dtime, username, sqlstring.Replace('\'','#').Replace('"', '#'), id);
            LogDB.Execute(sql);
        }

        private static string Parse(string sqlstring)
        {
            string result = sqlstring;
            try
            {
                int index = sqlstring.IndexOf("groupcomment");
                if (index != -1)
                {
                    int index2 = sqlstring.IndexOf("groupdeletereason");
                    if (index2 != -1)
                    {
                        string substrmain = sqlstring.Substring(index, index2 - index);
                        string comment = substrmain.Split('=')[1];
                        string substr = comment
                            .Substring(2, comment.Length - 5)
                            .Replace("#", "\\");
                        substr = RichTextStripper.StripRichTextFormat(substr);
                        string grcomment =
                            $" groupcomment = \"{substr}\", ";

                        result = result.Replace(substrmain, grcomment);
                    }
                }
            }
            catch(Exception ex)
